...
#618动物联萌
13 0-23/2 * * * sleep $((RANDOM % $RANDOM_DELAY_MAX)); node /scripts/jd_zoo.js |ts >> /scripts/logs/jd_zoo.log 2>&1
##############长期活动##############
# 签到
...
# 必须要的默认定时任务请勿删除
5 17 * * * docker_entrypoint.sh |ts >> /scripts/logs/default_task.log 2>&1
