...
0,30,31 20-23/1 * * * node /scripts/jd_live_redrain.js |ts >> /scripts/logs/jd_live_redrain.log 2>&1
...
0 0-18/6,19-23/1 * * * node /scripts/jd_carnivalcity.js |ts >> /scripts/logs/jd_carnivalcity.log 2>&1
##############长期活动##############
# 签到
...
# 京东全民开红包
12 1,21,23 * * * node /scripts/jd_redPacket.js |ts >> /scripts/logs/jd_redPacket.log 2>&1
# 进店领豆
...
# 必须要的默认定时任务请勿删除
46 23 * * * docker_entrypoint.sh |ts >> /scripts/logs/default_task.log 2>&1
