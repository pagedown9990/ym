...
#城城领现金
33 * * * * sleep $((RANDOM % $RANDOM_DELAY_MAX)); node /scripts/jd_city.js |ts >> /scripts/logs/jd_city.log 2>&1
...
#京东沸腾之夜(5.31日有效)
55,0 15-23/1 * * * sleep $((RANDOM % $RANDOM_DELAY_MAX)); node /scripts/jd_party_night.js |ts >> /scripts/logs/jd_party_night.log 2>&1
##############长期活动##############
# 签到
...
#京喜财富岛
5 */6 * * * sleep $((RANDOM % $RANDOM_DELAY_MAX)); node /scripts/jd_cfd.js |ts >> /scripts/logs/jd_cfd.log 2>&1
...
# 必须要的默认定时任务请勿删除
31 23 * * * docker_entrypoint.sh |ts >> /scripts/logs/default_task.log 2>&1
